                    ...
                    _data.delete('checks', checkId, err => {
                      ...
                      if (checksDeleted == checksToDelete) {
                        !deletionErrors
                          ...
                          : callback(500, {
                              Error:
                                'Errors encountered while attempting to delete all of the users checks. All checks mauy not have been deleted from the system successfully'
                            });
                      }
                    });
                  });
...
// Varify if a given token id is currently valid for a given user
    ...
    _data.read('checks', id, (err, checkData) => {
      if (!err && checkData) {
        ...
        handlers._tokens.verifyToken(token, checkData.userPhone, tokenIsValid =>
          tokenIsValid
            ? callback(200, checkData) // Return the check data
            : callback(403, {
                Error: 'Missing required token in header, or token is invalid'
              })
        );
